...
//		        "-fx-font-size: 14px;" + // Font size
	...
	public static GridPane createAttribute(ArrayList<Attribute> attributeList,int attributeType) {
		GridPane gridPane = new GridPane();
		Attribute dogAttribute = attributeList.get(0);
		String[] names = dogAttribute.getNames();
		for(int j = 0; j < names.length; j++) {
			// Add the label to the grid
			Label label = attributeLabel(names[j], gridPane, attributeList, attributeType, j );
